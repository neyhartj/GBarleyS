...
def rrBLUP_hapmap(VCF):
  ...
  for line in VCF:
    ...
    if line.startswith('##'):
      ...
      handle.write('rs#\talleles\tchrom\tpos\t' + '\t'.join(samples) + '\n')
...
def TASSEL_hapmap(VCF):
  ...
  for line in VCF:
    ...
    if line.startswith('##'):
      ...
      # First the header line with the information
      handle.write('rs#\talleles\tchrom\tpos\tstrand\tassembly#\tcenter\tprotLSID\tassayLSID\tpanelLSID\tQCcode\t' + '\t'.join(samples) + '\n')
